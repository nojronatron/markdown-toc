...
const { getSecondLevelHeading, getHash2LH, getDash2LH, findExistingStyleCharacter } = require('../../extension-functions/get-second-level-heading');

suite('getSecondLevelHeading Module tests', () => {
  test('getDash2LH returns alternate style L2 heading line, text, and isHash false', () => {
    ...
    const expected1 = { line: 2, text: 'Heading 1', isHash: false, isToc: false };
    ...
    const expected4 = { line: 5, text: 'Table of Contents', isHash: false, isToc: true };
  ...
  test('getHash2LH returns standard style L2 heading line, text, and isHash true', () => {
    const firstLine1 = '## Heading 1';
    const secondLine1 = '';
    const firstLineIdx1 = 2;
    // the standard heading style text will always be trimmed to just the text, no hashmarks
    ...
    const expected4 = { line: 4, text: '## Table of Contents', isHash: true, isToc: true };
  ...
  test('findExistingStyleCharacter returns the first character discovered (hash or dash), or a space character if neither could be found', ()=>{
    // Raw lorem ipsum text generated by [Lipsum.com](https://www.lipsum.com)
    
    const markdown1 = '# Top Heading\n\nLorem ipsum dolor sit amet, \nconsectetur adipiscing elit. \nPellentesque hendrerit tempus massa. \nCurabitur consectetur mollis scelerisque. \n\n## Heading 2\n\nUt at suscipit enim. \nSed convallis varius urna, \nquis scelerisque nisl facilisis at. \n';
    const expected1 = '#';
    const actual1 = findExistingStyleCharacter(markdown1);
    assert.strictEqual(actual1, expected1);

    const markdown2 = 'Top Heading\n=\n\nLorem ipsum dolor sit amet, \nconsectetur adipiscing elit. \nPellentesque hendrerit tempus massa. \nCurabitur consectetur mollis scelerisque. \n\nHeading 2\n-\n\nUt at suscipit enim. \nSed convallis varius urna, \nquis scelerisque nisl facilisis at. \n';
    const expected2 = '-';
    const actual2 = findExistingStyleCharacter(markdown2);
    assert.strictEqual(actual2, expected2);

    const markdown3 = 'Lorem ipsum dolor sit amet, \nconsectetur adipiscing elit. \nPellentesque hendrerit tempus massa. \nCurabitur consectetur mollis scelerisque. \nUt at suscipit enim. \nSed convallis varius urna, \nquis scelerisque nisl facilisis at. \nQuisque semper massa nec lobortis varius. \nNulla feugiat tristique ante, in bibendum lacus ornare id. \nNulla nec suscipit turpis. \nEtiam non mollis velit. \nNullam vulputate feugiat dui, \nsed suscipit dui consectetur ut. \nNulla facilisi. \nQuisque ut neque iaculis, \nhendrerit urna sed, \nrhoncus risus. \nDuis ac nulla sodales justo vestibulum aliquam a eget mi.';
    const expected3 = '';
    const actual3 = findExistingStyleCharacter(markdown3);
    assert.strictEqual(actual3, expected3);

    const markdown4 = 'Heading 1\n\nLorem ipsum dolor sit amet, \nconsectetur adipiscing elit. \n\nHeading 2\n\nPellentesque hendrerit tempus massa. \nCurabitur consectetur mollis scelerisque.\n\nHeading 3\n\nUt at suscipit enim. \nSed convallis varius urna, \nquis scelerisque nisl facilisis at. \n';
    const expected4 = '';
    const actual4 = findExistingStyleCharacter(markdown4);
    assert.strictEqual(actual4, expected4);
  });

  test('getSecondLevelHeading returns correct object when standard style L2 heading is passed-in', () => {
    ...
    const actualResult1 = getSecondLevelHeading(standardLine1, 0, ' ', true);
    assert.strictEqual(actualResult1.line, expectedStandard1.line);
    assert.strictEqual(actualResult1.text, expectedStandard1.text);
    assert.strictEqual(actualResult1.isHash, expectedStandard1.isHash);
    assert.strictEqual(actualResult1.isToc, expectedStandard1.isToc);
    ...
    assert.strictEqual(actualresult2.line, expectedStandard2.line);
    assert.strictEqual(actualresult2.text, expectedStandard2.text);
    assert.strictEqual(actualresult2.isHash, expectedStandard2.isHash);
    assert.strictEqual(actualresult2.isToc, expectedStandard2.isToc);

    const expectedStandard3 = { line: -1, text: '### Level 3 Heading\n', isHash: true, isToc: false };
    const actualResult3 = getSecondLevelHeading(standardLine3, 8, ' ', true);
    assert.strictEqual(actualResult3.line, expectedStandard3.line);
    assert.strictEqual(actualResult3.text, expectedStandard3.text);
    assert.strictEqual(actualResult3.isHash, expectedStandard3.isHash);
    assert.strictEqual(actualResult3.isToc, expectedStandard3.isToc);
    ...
    assert.strictEqual(actualResult4.line, expectedStandard4.line);
    assert.strictEqual(actualResult4.text, expectedStandard4.text);
    assert.strictEqual(actualResult4.isHash, expectedStandard4.isHash);
    assert.strictEqual(actualResult4.isToc, expectedStandard4.isToc);
  });
